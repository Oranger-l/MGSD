...
# Student starts from the 8B perception-SFT model; Teacher/ref starts from the 8B base model.
MODEL_PATH=${MODEL_PATH:-${REPO_ROOT}/models/ckpts/Qwen3-VL-8B-VSP-Tasks-Perception-SFT-Final}
TEACHER_MODEL_PATH=${TEACHER_MODEL_PATH:-${REPO_ROOT}/models/Qwen3-VL-8B-Instruct}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen3vl8b_vsp_tasks_mixed_opcd_base_teacher_sft_student_final}
DATA_SEED=${DATA_SEED:-1}

# Batch/runtime knobs. Defaults match the 4B base-teacher script.
ROLLOUT_BATCH_SIZE=${ROLLOUT_BATCH_SIZE:-32}
GLOBAL_BATCH_SIZE=${GLOBAL_BATCH_SIZE:-32}
ACTOR_MICRO_BATCH_EXPERIENCE=${ACTOR_MICRO_BATCH_EXPERIENCE:-8}
ACTOR_MICRO_BATCH_UPDATE=${ACTOR_MICRO_BATCH_UPDATE:-4}
ROLLOUT_GPU_MEMORY_UTILIZATION=${ROLLOUT_GPU_MEMORY_UTILIZATION:-0.90}
