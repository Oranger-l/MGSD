...
class DataConfig:
    ...
    """column name for teacher extra images in dataset (e.g. 'teacher_images')"""
    teacher_text_key: Optional[str] = None
    """column name for teacher-only text context (e.g. chart generation code)"""
    return_raw_chat: bool = False
    """store raw chat messages in batch for re-tokenization (needed for OPCD)"""
    system_prompt: Optional[str] = None
    """file path to system prompt to prepend to messages (loaded at dataset init)"""
    prompt_image_paths: Optional[str] = None
    """comma-separated static image paths to prepend to the student prompt flow"""
    ...
    def post_init(self):
        ...
        if self.prompt_image_paths:
            resolved_prompt_image_paths = []
            for raw_path in self.prompt_image_paths.split(","):
                raw_path = raw_path.strip()
                if not raw_path:
                    continue
                resolved_prompt_image_paths.append(
                    get_abs_path(raw_path, prompt="Prompt image path") or os.path.abspath(os.path.expanduser(raw_path))
                )
            self.prompt_image_paths = ",".join(resolved_prompt_image_paths)
...
class AlgorithmConfig:
    ...
    use_mm_context_distillation: bool = False
    """enable multimodal context distillation (OPCD)"""
    teacher_system_prompt: Optional[str] = None
    """file path to teacher system prompt for OPCD (avoids YAML parsing issues with special chars)"""
    teacher_append_ground_truth: bool = True
    """whether to append ground-truth answer text to teacher inputs for OPCD"""
    only_reverse_kl_advantages: bool = False
    """use reverse KL as advantages: advantages = -(old_log_prob - ref_log_prob)"""
