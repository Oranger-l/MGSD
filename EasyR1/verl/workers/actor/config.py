...
class RefConfig:
    strategy: str = "fsdp"
    sync_with_actor: bool = False
    """share actor weights for reference/teacher log-prob computation"""
    model: ModelConfig = field(default_factory=ModelConfig)
    """reference/teacher model config; defaults to actor.model for backward compatibility"""
    fsdp: FSDPConfig = field(default_factory=FSDPConfig)
    offload: OffloadConfig = field(default_factory=OffloadConfig)
    # below are auto keys
