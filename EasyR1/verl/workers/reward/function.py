...
class SequentialFunctionRewardManagerMixin:
    ...
    @staticmethod
    def _get_optional_non_tensor_value(data: DataProto, key: str, idx: int):
        if key not in data.non_tensor_batch:
            return None
        value = data.non_tensor_batch[key][idx]
    ...
    def compute_reward_sequential(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        for i in range(len(data)):
            ...
            frozenlake_task_text = self._get_optional_non_tensor_value(data, "frozenlake_task_text", i)
            if frozenlake_task_text is not None:
                reward_input["frozenlake_task_text"] = frozenlake_task_text
            task = self._get_optional_non_tensor_value(data, "task", i)
            if task is not None:
                reward_input["task"] = task
            vsp_task_text = self._get_optional_non_tensor_value(data, "vsp_task_text", i)
            if vsp_task_text is not None:
                reward_input["vsp_task_text"] = vsp_task_text
...
class BatchFunctionRewardManagerMixin:
    ...
    def compute_reward_batch(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        for i in range(len(data)):
            ...
            frozenlake_task_text = SequentialFunctionRewardManagerMixin._get_optional_non_tensor_value(
                data, "frozenlake_task_text", i
            )
            if frozenlake_task_text is not None:
                reward_input["frozenlake_task_text"] = frozenlake_task_text
            task = SequentialFunctionRewardManagerMixin._get_optional_non_tensor_value(data, "task", i)
            if task is not None:
                reward_input["task"] = task
            vsp_task_text = SequentialFunctionRewardManagerMixin._get_optional_non_tensor_value(
                data, "vsp_task_text", i
            )
            if vsp_task_text is not None:
                reward_input["vsp_task_text"] = vsp_task_text
