...
CONFIG_PATH="${CONFIG_PATH:-examples/train_lora/qwen3vl_8b_vsp_tasks_perception_sft.yaml}"
MODEL_NAME_OR_PATH="${MODEL_NAME_OR_PATH:-${REPO_ROOT}/models/Qwen3-VL-8B-Instruct}"
OUTPUT_DIR="${OUTPUT_DIR:-${LLAMAFACTORY_ROOT}/saves/qwen3-vl-8b/vsp_tasks_perception_lora_sft}"
...
WANDB_NAME="${WANDB_NAME:-qwen3vl8b_vsp_tasks_perception_sft_final}"
SWANLAB_API_KEY="${SWANLAB_API_KEY:-}"
SWANLAB_PROJECT="${SWANLAB_PROJECT:-VSP-Tasks}"
SWANLAB_RUN_NAME="${SWANLAB_RUN_NAME:-qwen3vl8b_vsp_tasks_perception_sft_final}"
...
RUN_CONFIG_PATH="$(mktemp /tmp/qwen3vl8b_vsp_tasks_perception_sft.XXXXXX.yaml)"
...
if [[ "$ENABLE_SWANLAB_RUNTIME" == "true" ]]; then
    echo "SwanLab enabled: project=${SWANLAB_PROJECT:-VSP-Tasks}, run=${SWANLAB_RUN_NAME:-qwen3vl8b_vsp_tasks_perception_sft_final}"
fi
